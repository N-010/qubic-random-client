...
This repository is a minimal Rust binary crate.
- `src/` holds application code. Entry point is `src/main.rs`.
- `Cargo.toml` defines the crate name and dependencies.
- `Cargo.lock` is committed for deterministic builds.
- `target/` contains build artifacts (do not edit manually).
- `docs/Random.h` is the SC interface header the client will target.
- `docs/ShortDescriptio.txt` contains a brief description of the SC behavior.
- `docs/Architecture.md` contains the client architecture overview.
...
- `cargo test` — run tests (none are defined yet).
- `cargo clean` — remove `target/` artifacts.

## Coding Style & Naming Conventions
Follow Rust defaults unless the project adds tooling later:
- Indentation: 4 spaces, no tabs (rustfmt default).
- Naming: `snake_case` for functions/modules, `PascalCase` for types, `SCREAMING_SNAKE_CASE` for constants.
- Keep `main.rs` small; extract logic into modules as the project grows (e.g., `src/lib.rs`, `src/bin/`).

## Testing Guidelines
There are currently no tests. When adding them:
- Prefer Rust’s built-in test framework with `#[test]` functions in `src/` or `tests/`.
- Name test modules and functions descriptively (e.g., `tests/parse_input.rs`, `test_parse_empty_input`).
- Run `cargo test` before submitting changes.

## Commit & Pull Request Guidelines
No Git history exists yet, so there is no established commit convention.
- Use short, imperative commit messages (e.g., `Add input parsing`).
- Pull requests should include a concise description, what changed, and how to verify.
- If you add user-visible behavior, include a brief test plan and expected output.
