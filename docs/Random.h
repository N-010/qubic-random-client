	...
	BEGIN_TICK_WITH_LOCALS()
	{
		...
		if (locals.i == state.populations.get(locals.stream)) // Nobody provided their reveal, that tick was probably empty
		{
			while (locals.i--)
			{
				...
				{
				case 0: qpi.transfer(state.providers.get(locals.stream * 1365 + locals.i), 1); break;

				case 1: qpi.transfer(state.providers.get(locals.stream * 1365 + locals.i), 10); break;

				case 2: qpi.transfer(state.providers.get(locals.stream * 1365 + locals.i), 100); break;

				case 3: qpi.transfer(state.providers.get(locals.stream * 1365 + locals.i), 1000); break;

				case 4: qpi.transfer(state.providers.get(locals.stream * 1365 + locals.i), 10000); break;

				case 5: qpi.transfer(state.providers.get(locals.stream * 1365 + locals.i), 100000); break;

				case 6: qpi.transfer(state.providers.get(locals.stream * 1365 + locals.i), 1000000); break;

				case 7: qpi.transfer(state.providers.get(locals.stream * 1365 + locals.i), 10000000); break;

				case 8: qpi.transfer(state.providers.get(locals.stream * 1365 + locals.i), 100000000); break;

				default: qpi.transfer(state.providers.get(locals.stream * 1365 + locals.i), 1000000000);
				}
				state.providers.set(locals.stream * 1365 + locals.i, id::zero());
				state.collateralTiers.set(locals.stream * 1365 + locals.i, 0);
				// Don't need to zero [state.reveals], they are all-zeros already
